...
| Archive List | 기록 검색/장르 분포/정리 | 검색, 장르 비율 바, 다중 선택 삭제 |
...
4. 팝업 확인: 체크박스, 왼쪽 장르명, 오른쪽 연관성(`강함`, `보통`, `약함`) 리스트 표시
5. 지도 반영: 선택한 후보만 `선택 반영`으로 주변 노드와 강도별 연결선 추가
...
### 7.3 Archive List
- 검색바 + 장르 비율 바
- 장르 비율 바는 보관함 앨범의 대표 장르 분포를 누적 프로그레스 형태로 표시한다.
- 사용자가 바 구간 또는 범례를 누르면 해당 장르의 비율과 앨범 수를 즉시 표시한다.
- `전체/최근/재즈/일렉트로닉` 같은 고정 필터 칩은 사용하지 않는다.
- 롱프레스로 선택 모드 진입
- 취소/삭제 하단 액션바

### 7.4 Genre Stats
- 현재 구현: 전체 보관함 기준 도넛 + KPI
- 기간 세그먼트와 주간 막대 차트는 실제 기간 집계 DAO가 추가될 때 연결
...
- 테마: 라이트 모드 / 다크 모드 / 시스템 기본값 선택을 저장하고 즉시 앱 테마에 반영
- 데이터 관리: 앨범 커버 캐시 삭제와 저장 기록 초기화를 실제 동작에 연결하고 결과 메시지를 표시
